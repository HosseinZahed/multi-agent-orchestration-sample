...
async def on_message(user_message: cl.Message):
    ...
    # Define the Chainlit filter for function invocation
    async def chainlit_function_invocation_filter(context: FunctionInvocationContext, next):
        if "messages" not in context.arguments:
            await next(context)
            return
        await cl.Message(
            author=f"Function Call: {context.function.name}",
            content=f"Arguments: {context.arguments['messages']}"
        ).send()
        await next(context)
        await cl.Message(
            author=f"Function Response: {context.function.name}",
            content=f"Result: {context.result.value}"
        ).send()
